...
(() => {
 ...
 function miniAttention(values,x,y){const size=Math.min(7,48/values.length-1);return values.map((row,i)=>row.map((v,j)=>`<rect x="${x+j*(size+1)}" y="${y+i*(size+1)}" width="${size}" height="${size}" rx="1" fill="${j>i?'var(--line)':`rgba(43,119,111,${.12+v*.88})`}"/>`).join('')).join('');}
 function diagram(m,stage,ranked){
  ...
  return `<svg viewBox="0 0 1100 320" role="img" aria-label="Computed decoder path: token IDs, embeddings and positions, pre-normalized two-head causal attention, two residual additions, MLP, final normalization, output logits."><defs><marker id="tf-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0L8 4L0 8" fill="none" stroke="currentColor"/></marker></defs>
   ...
   ${block(18,63,125,198,'Token IDs',m.ids.length+' positions',stage===0)}${m.ids.map((id,i)=>`<text x="32" y="${113+i*Math.min(21,120/(m.ids.length-1||1))}" class="tf-svg-token">${VOCAB[id]}</text><text x="126" y="${113+i*Math.min(21,120/(m.ids.length-1||1))}" text-anchor="end" class="tf-svg-meta">${id}</text>`).join('')}${arrow(146,172,155)}
   ${block(181,63,149,198,'Embeddings',m.ids.length+' × 8 · E + position',stage===0)}${miniMatrix(m.x,200,108,Math.min(12,120/m.ids.length-2))}${arrow(333,360,155)}
 ...
 function mount(root){
  ...
  let ids=EXAMPLES[0].slice(),m=forward(ids),stage=1,query=4,key=1,head=0,temp=1,topk=16,sampleSeed=117,experiment='causal',comparison=null;
  root.innerHTML=`<header class="tf-header"><div><span class="section-number">03 / INSIDE THE TRANSFORMER</span><h2>Follow a token. Inspect every step.</h2><p>Transformer anatomy, from an embedding lookup to the next-token distribution.</p></div><span class="tf-computed"><i></i>Computed in your browser</span></header>
   ...
   <div class="tf-experiments"><p class="tf-experiments-label">GUIDED EXPERIMENTS / CHANGE ONE THING</p><div class="tf-experiment-buttons"><button data-tf-experiment="causal" aria-pressed="true">01 · Change a future token</button><button data-tf-experiment="heads" aria-pressed="false">02 · Compare attention heads</button><button data-tf-experiment="temperature" aria-pressed="false">03 · Reshape a distribution</button></div><div class="tf-experiment-result" data-tf-experiment-result aria-live="polite"></div></div><nav class="tf-stages" aria-label="Inspect a transformer computation">${STAGES.map(([name],i)=>`<button type="button" data-tf-stage="${i}" aria-pressed="${i===stage}"><small>0${i+1}</small>${name}<span>↗</span></button>`).join('')}</nav>
  ...
  function attention(){
   ...
   return `<div class="tf-attention-layout"><figure class="tf-heatmap"><figcaption><strong>Attention weights</strong><span>Key position → · query position ↓</span></figcaption><div class="tf-matrix" style="--n:${n}"><span></span>${ids.map((_,i)=>`<span class="tf-axis">${i}</span>`).join('')}${ids.map((_,i)=>`<span class="tf-axis">${i}</span>${ids.map((_,j)=>`<button data-tf-cell="${i},${j}" class="${j>i?'masked':''} ${h.attention[i][j]>.93?'high-weight':''} ${i===query?'query-row':''}" aria-pressed="${i===query&&j===key}" aria-label="Query ${i} ${VOCAB[ids[i]]}, key ${j} ${VOCAB[ids[j]]}: ${j>i?'masked, zero weight':pct(h.attention[i][j])}" style="--weight:${.13+.87*h.attention[i][j]}">${j>i?'×':Math.round(h.attention[i][j]*100)}</button>`).join('')}`).join('')}</div><div class="tf-legend"><span>Weights in %</span><span><i></i>0 → 100</span><span>× Future masked</span></div><p class="tf-row-sum">Selected row sum: <strong>${h.attention[query].reduce((s,v)=>s+v,0).toFixed(6)}</strong></p></figure>
  ...
  function guide(){
   root.querySelectorAll('[data-tf-experiment]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.tfExperiment===experiment)));
   const out=el('[data-tf-experiment-result]');
   if(experiment==='causal')out.innerHTML=comparison?`<strong>Earlier positions: maximum logit change ${comparison.earlier.toFixed(6)}.</strong> Final position: maximum change ${comparison.final.toFixed(3)}. Only the last input changed (${comparison.from} → ${comparison.to}). Causal masking preserves every earlier position’s output.<button data-tf-future>Change the final token again →</button>`:'<strong>Can a future token change an earlier prediction?</strong> Change the last token while keeping the prefix fixed. Compare all earlier logits before and after the forward pass.<button data-tf-future>Change the final token →</button>';
   if(experiment==='heads')out.innerHTML=`<strong>Same query, different projections.</strong> For query ${query} (“${VOCAB[ids[query]]}”) and key ${key} (“${VOCAB[ids[key]]}”), head 1 assigns ${pct(m.heads[0].attention[query][key])} and head 2 assigns ${pct(m.heads[1].attention[query][key])}. Switch heads and inspect the Q/K vectors. With untrained weights, these differences do not represent learned linguistic roles.`;
   if(experiment==='temperature'){
    const entropy=t=>distribution(m.logits.at(-1),t,16).reduce((sum,r)=>sum-(r.p?r.p*Math.log2(r.p):0),0);
    out.innerHTML=`<strong>Temperature changes concentration, not logit order.</strong> With all 16 candidates, entropy is ${entropy(.4).toFixed(2)} bits at T = 0.4 and ${entropy(1.4).toFixed(2)} bits at T = 1.4. Higher entropy spreads probability across more candidates; it does not establish answer quality.<button data-tf-temperature-example>Set T to ${temp<1?'1.4':'0.4'} →</button>`;
   }
  }
  function update(){
   const focused=document.activeElement,attrs=['data-tf-cell','data-tf-temperature','data-tf-k','data-tf-generate','data-tf-reset','data-tf-future','data-tf-temperature-example'];const restore=attrs.find(a=>focused&&focused.hasAttribute(a)),restoreValue=restore?focused.getAttribute(restore):null;
   ...
   el('[data-tf-detail]').innerHTML=[embeddings,attention,residual,decoding][stage]();guide();
   if(restore)el(`[${restore}="${restoreValue}"]`)?.focus({preventScroll:true});
  }
  function reset(){ids=EXAMPLES[+el('[data-tf-prompt]').value].slice();m=forward(ids);query=4;key=1;sampleSeed=117;comparison=null;sequence();update();}
  root.addEventListener('click',e=>{
   const b=e.target.closest('button');if(!b)return;
   if(b.hasAttribute('data-tf-experiment')){
    experiment=b.dataset.tfExperiment;
    if(experiment==='temperature'){stage=3;temp=.4;topk=16;}else{stage=1;query=Math.min(4,ids.length-2);key=1;}
   }
   else if(b.hasAttribute('data-tf-future')){
    const previous=m,from=ids.at(-1);ids[ids.length-1]=from===9?8:9;m=forward(ids);
    const change=(row,i)=>Math.max(...row.map((v,j)=>Math.abs(v-previous.logits[i][j])));
    comparison={earlier:Math.max(...m.logits.slice(0,-1).map(change)),final:change(m.logits.at(-1),ids.length-1),from:VOCAB[from],to:VOCAB[ids.at(-1)]};
    stage=1;query=ids.length-2;key=ids.length-1;sequence();
   }
   else if(b.hasAttribute('data-tf-temperature-example')){stage=3;temp=temp<1?1.4:.4;topk=16;}
   else if(b.hasAttribute('data-tf-token'))query=+b.dataset.tfToken;
