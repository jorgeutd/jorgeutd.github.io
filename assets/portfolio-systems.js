...
(()=>{
 ...
 function controls(){return `<div class="systems-controls">
  ...
  <label>Request order<select data-sys-input="order"><option value="media-first">Media burst, then text</option><option value="interleaved">Evenly interleaved</option></select></label>
 ...
 function inference(root){
  let config={...M.defaults},selected=19;
  root.innerHTML=`<section class="systems-lab" aria-label="Inference topology simulator"><div class="systems-cap"><span>01 / CAPACITY &amp; LATENCY</span><b>Computed teaching model</b></div><div class="systems-presets"><button data-sys-preset="mixed" class="pill">Protect text latency</button><button data-sys-preset="media" class="pill">Image-heavy workload</button><button data-sys-preset="decode" class="pill">Long responses</button><button data-sys-preset="text" class="pill">Text-only traffic</button></div>${controls()}<div data-sys-results></div><details class="systems-contract"><summary>Assumptions, equations and exact request records</summary><p>32 deterministic requests; FIFO queues; one request at a time per worker; identical GPU capacity; no batching, KV reuse, network contention or cold starts. These are simulated milliseconds, not model benchmarks. The encoder split changes scheduling and placement; prefill and decode remain together. A reserved encoder GPU stays reserved even in the text-only experiment.</p><p>Encode = 25 ms × images. Prefill = (12 + 5 × images) / acceleration, with zero images for text. Decode = 1.6 ms × output tokens / acceleration. Transfer is charged only to image requests. First token arrives after prefill plus one decode step. A 2× LLM speed setting is a hypothetical service-time change; it establishes no quantization accuracy or hardware speedup.</p><p>p95 uses nearest rank over all 32 requests. The completion target is 1,000 ms from arrival, including queueing. This model omits the batching and hardware-efficiency gains that can favor separation in real systems. It is designed to expose queue isolation and reserved-capacity tradeoffs, not select a deployment. A GPU budget alone is not a complete cost model. Every configuration should be re-evaluated with an actual serving engine, batching, realistic traffic and task-quality checks.</p><pre data-sys-records></pre>${jump('Read the compact engineering note →','/notes/inference-topology/')}</details></section>`;
  ...
  function update(){
   ...
   lab.querySelector('[data-sys-results]').innerHTML=`<div class="systems-verdict" role="status"><strong>${Math.abs(delta)<.1?'The two layouts finish within 0.1%.':`The split layout has ${Math.abs(delta).toFixed(0)}% ${delta<0?'lower':'higher'} p95 completion latency.`}</strong><span>${a.p95Text!==null?`Text p95 first token: ${ms(a.p95Text)} → ${ms(b.p95Text)}. `:''}Same ${config.gpus}-GPU budget · ${config.count} synthetic requests.</span></div><div class="systems-compare">${[['aggregated','Shared workers',a,`${config.gpus} GPUs · each runs encode + prefill + decode`],['separated','Separate encoder pool',b,`${config.encoders} encoder + ${config.gpus-config.encoders} LLM GPUs · same total`]].map(([key,title,r,sub])=>`<section class="systems-topology" data-sys-layout="${key}"><div class="systems-topology-head"><span class="tag">${key==='aggregated'?'A / COMBINED':'B / SPLIT'}</span><h2>${title}</h2><p>${sub}</p></div><div class="systems-path">${(key==='aggregated'?['One queue','Encode → prefill → decode','Response']:['Media → encode → transfer','Text bypasses encoder','LLM queue → prefill → decode']).map((v,i)=>`<div><small>0${i+1}</small><b>${v}</b></div>`).join('')}</div><div class="systems-measures"><div><strong data-sys-metric="first">${ms(r.p95First)}</strong><span>p95 first token</span></div><div><strong data-sys-metric="end">${ms(r.p95End)}</strong><span>p95 completion</span></div><div><strong data-sys-metric="text">${r.p95Text===null?'N/A':ms(r.p95Text)}</strong><span>p95 first token · text only</span></div><div><strong>${r.passed} / ${config.count}</strong><span>complete within 1 second</span></div></div></section>`).join('')}</div><div class="systems-trace-heading"><h3>Follow the same request.</h3><label>Request<select data-sys-request>${result.requests.map(r=>`<option value="${r.id}" ${r.id===selected?'selected':''}>${String(r.id+1).padStart(2,'0')} · ${r.media?'images':'text'} · arrives ${ms(r.arrival)}</option>`).join('')}</select></label></div><div class="systems-compare systems-timelines"><section><h4>Shared workers</h4>${timeline(a.rows[selected],max)}</section><section><h4>Separate encoder pool</h4>${timeline(b.rows[selected],max)}</section></div>`;
  ...
  lab.addEventListener('input',e=>{const k=e.target.dataset.sysInput;if(!k)return;config[k]=k==='order'?e.target.value:+e.target.value;if(k==='gpus'){config.encoders=Math.min(config.encoders,config.gpus-1);sync();}update();});
  ...
  lab.addEventListener('click',e=>{const p=e.target.closest('[data-sys-preset]')?.dataset.sysPreset;if(!p)return;config={...M.defaults,...({mixed:{},media:{media:100,images:12,tokens:32},decode:{media:50,images:2,tokens:2048},text:{media:0,tokens:128}}[p])};sync();update();});
  sync();update();
 }
